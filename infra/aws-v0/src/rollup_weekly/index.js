...
const GLOBAL_TABLE_NEW = process.env.GLOBAL_TABLE_NEW;
const MIGRATE_GLOBAL_NEW_TO_CANONICAL = process.env.MIGRATE_GLOBAL_NEW_TO_CANONICAL === "true";
...
const WEEKLY_TOTAL_KEYS = {
  ...
  shield_opens: "total_shield_opens",
...
function sumMetrics(items) {
  ...
  for (const item of items) {
    for (const key of METRIC_KEYS) {
      const n = typeof item[key] === "number" ? item[key] : 0;
      totals[key] += n;
    }
  }
  return totals;
...
async function migrateGlobalNewToCanonical() {
  if (!GLOBAL_TABLE_NEW) {
    throw new Error("GLOBAL_TABLE_NEW not set");
  }
  let lastKey = undefined;
  do {
    const scan = new ScanCommand({
      TableName: GLOBAL_TABLE_NEW,
      ExclusiveStartKey: lastKey,
    });
    const resp = await docClient.send(scan);
    const items = resp.Items || [];
    for (const item of items) {
      if (!item.metric_name || !item.period) continue;
...
exports.handler = async (event = {}) => {
  if (MIGRATE_GLOBAL_NEW_TO_CANONICAL) {
    await migrateGlobalNewToCanonical();
